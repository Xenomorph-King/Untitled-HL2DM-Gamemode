...
function WSWITCH:DrawBarBg(x, y, w, h, col)
   ...
   local role = LocalPlayer():getTeam()
   ...
   surface.DrawRect( rx+b, ry, h+8, rh )
...
function WSWITCH:DrawWeapon(x, y, c, wep)
   ...
   -- Slot
   local spec = {text=" "..wep.Slot+1, font="Borg40", pos={x+12, y}, yalign=TEXT_ALIGN_CENTER, color=c.text}
   draw.TextShadow(spec, 1, c.shadow)

   -- Name
   spec.text  = name:upper()
   spec.font  = "Borg30"
   spec.pos[1] = x + 23 + height
   draw.Text(spec)

   if ammo and not hide then
      local col = c.text
...
function WSWITCH:Draw(client)
   ...
   local y = ScrH() - (#weps * (height + margin)) - 180
   if client:GetActiveWeapon() and client:GetActiveWeapon():IsValid() and client:GetActiveWeapon():GetClass() == "weapon_liro_ar2" then
      y=y-70
   end
   local col = col_dark
...
function WSWITCH:SelectSlot(slot)
   ...
   for k, w in pairs(self.WeaponCache) do
      if w.Slot == slot and self.Selected != k then
         toselect = k
         break
      end
   end
...
function WSWITCH:ConfirmSelection(noHide)
   ...
   for k, w in pairs(self.WeaponCache) do
      if k == self.Selected and IsValid(w) then
         ...
         RunConsoleCommand("wepswitch2", w:GetClass())
         return
      end
   end
end
